...
function _ppr_get_js() {
	...
	$js_printed = true;
	$js = <<<JS
<script>(function(win) {var eachOnclick=document.querySelectorAll ?function(parent, className, onclick) {var els=parent.querySelectorAll('.' + className);for (var i=0, len=els.length; i < len; i++) {els[i].onclick=onclick;}} :function(parent, className, onclick) {var all=parent.getElementsByTagName('*');var matcher=new RegExp("\\b" + className + "\\b");for (var i=0, len=all.length; i < len; i++) {if (matcher.test(all[i].className)) {all[i].onclick=onclick;}}};var hide=function(el) { el.style.display='none'; };var show=function(el) { el.style.display=''; };var toggleCollapsed=function() {var collapsible=this.nextSibling.nextSibling;if (collapsible.style.display=='none') {this.className=this.className.replace(' ppr-collapsed', '');show(collapsible); hide(collapsible.nextSibling); }else {this.className+=' ppr-collapsed';hide(collapsible); show(collapsible.nextSibling); }};var more=function() {hide(this); hide(this.previousSibling); show(this.nextSibling); show(this.nextSibling.nextSibling); };var less=function() {var el;hide(this); hide(el=this.previousSibling); show(el=el.previousSibling); show(el.previousSibling); };win.pprDecorate=function() {var pres=document.getElementsByTagName('pre');var pre=pres[pres.length-1];eachOnclick(pre, 'ppr-collapser', toggleCollapsed);eachOnclick(pre, 'ppr-more', more);eachOnclick(pre, 'ppr-less', less);};win.pprDecorate();}(window));</script>
JS;
	return $js;
}
